...
import { Firestore, collection, collectionData, addDoc, updateDoc, deleteDoc, doc, runTransaction } from '@angular/fire/firestore';
  ...
  drop(event: CdkDragDrop<Task[] | null>): void {
    ...
    runTransaction(this.store, transaction => {
      const promise = Promise.all([
        transaction.delete(doc(this.store, event.previousContainer.id, item.id || '')),
        transaction.set(doc(this.store, event.container.id, item.id || ''), item),
      ]);
      return promise;
    });
